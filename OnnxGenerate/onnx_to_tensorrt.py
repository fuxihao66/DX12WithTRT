...
with get_engine("./UNetLWGated_Multi.onnx","./UNetLWGated_Multi_onnx_trt_fp16.pb") as engine, engine.create_execution_context() as context:
    with torch.no_grad():
        ...
        finalMask = torch.tensor(finalMask).unsqueeze(0).to(mdevice)
        labelimg = torch.tensor(labelimg).unsqueeze(0).to(mdevice)

        inputs, outputs, bindings, stream = common.allocate_buffers(engine)

        inputs[3].host = hisBuffer.cpu().numpy().copy()
        ...
        cv.imwrite("./resTrain/res train fp16 %s.exr"%("trt"), NP2NP(output))
